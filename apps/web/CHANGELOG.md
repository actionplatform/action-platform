...
- **web:** version increment as a switch — Patch / Minor / Major with the resulting version under each
- **web:** New deployment without the target card
...
### Refactoring
- **web:** release and deploy forms on one grid — two columns, same selects, same summary strip
- **web:** one shape for the action forms — ActionForm, useAction, SegmentedControl
