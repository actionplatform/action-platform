  ...
  return (
    <form onSubmit={(e) => { e.preventDefault(); start(async () => { setError(null); finish(await addApp(projectId, url), false); }); }} className="space-y-2">
      ...
      <Dialog
        ...
        footer={<><Button variant="ghost" onClick={() => setNeedsInstall(false)} disabled={pending}>Cancel</Button><Button disabled={pending} onClick={() => start(async () => { setError(null); finish(await addApp(projectId, url, { type, ci }), true); })}><Wrench className="size-4" strokeWidth={1.75} /> {pending ? "Installing…" : "Install and add"}</Button></>}
      >
        ...
        <p className="mt-3 text-xs text-muted-foreground">The language is detected from the repository (pyproject.toml, go.mod, package.json…).</p>
      </Dialog>
    </form>
  );
}
