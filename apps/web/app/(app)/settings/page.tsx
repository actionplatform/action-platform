...
export default async function SettingsPage({ searchParams }: { searchParams: Promise<{ connected?: string; oauth_error?: string; github_app?: string }> }) {
  ...
  const [members, invitations, hosts, query, author] = await Promise.all([membersOf(), canManage ? invitationsOf() : Promise.resolve([]), hostsOf(org.id), searchParams, gitAuthorOf()]);
