...
import { membersOf, roleOf } from "@/lib/orgs";
...
export default async function TeamPage({ params }: { params: Promise<{ team: string }> }) {
  ...
  const [members, orgMembers, projects, allProjects, role] = await Promise.all([teamMembersOf(team.id), membersOf(org.id), projectsOfTeam(org.id, team.id), projectsOf(org.id), roleOf(session.user.id, org.id)]);
