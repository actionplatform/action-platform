...
export async function GET(req: Request) {
  ...
  try {
    page = await v1.githubManifest({ origin: publicOrigin(req.headers), host: url.host, return_to: safePath(url.searchParams.get("return"), "/settings/integrations"), github_org: url.searchParams.get("org")?.trim() || "" });
  } catch (e) {
