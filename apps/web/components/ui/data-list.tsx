...
export function DataList<T>({ title, rows: all, rowKey, columns, card, noun, meta, action, newHref, newLabel = "New", empty, rowClassName, after, minWidth = "640px" }: Props<T>) {
  const rows = all;
      ...
      {all.length === 0 ? (
        ...
        <>
          <div className="hidden overflow-x-auto md:block">
            <table className="w-full text-sm" style={{ minWidth }}>
              ...
              <tbody className="divide-y divide-border-subtle border-t border-border-subtle">
                {rows.map((row, i) => [
                  <tr key={rowKey(row)} className={cn("align-middle", rowClassName?.(row))}>{columns.map((c) => <td key={c.key} className={cn("py-2.5 pr-4 first:px-4 last:pr-3", c.className)}>{c.render(row, i)}</td>)}</tr>,
                  after?.(row, i),
                ])}
              </tbody>
            </table>
          ...
          <ul className="divide-y divide-border-subtle border-t border-border-subtle md:hidden">
            {rows.map((row, i) => <li key={rowKey(row)} className="px-4 py-3 text-sm">{card(row, i)}</li>)}
          </ul>
        </>
      )}
    </Panel>
