    ...
    <div className="fixed inset-0 z-40 flex items-center justify-center bg-background/80 p-4" onMouseDown={(e) => { if (e.target === e.currentTarget) onClose(); }}>
      <div role="dialog" aria-modal aria-labelledby={id} className={`max-h-[calc(100vh-2rem)] w-full max-w-md overflow-y-auto rounded-lg border border-border bg-surface ${className ?? ""}`}>
        <div className="flex items-start justify-between gap-4 px-4 pt-4">
          ...
          <button type="button" onClick={onClose} className="text-secondary hover:text-foreground" aria-label="Close"><X className="size-4" /></button>
        </div>
    ...
    <Dialog
      ...
      footer={
        <>
          ...
          <Button variant={danger ? "destructive" : "default"} onClick={onConfirm} disabled={pending}>{pending ? "Working…" : confirmLabel}</Button>
        </>
      }
    >
      {children}
    ...
    <Dialog
      ...
      footer={
        <>
          ...
          <Button onClick={() => onSubmit(value)} disabled={pending || !value.trim()}>{pending ? "Working…" : submitLabel}</Button>
        </>
      }
    >
      <form onSubmit={(e) => { e.preventDefault(); if (value.trim()) onSubmit(value); }}>
        <Field label={label} hint={hint}>
          <Input type={type} value={value} onChange={(e) => setValue(e.target.value)} placeholder={placeholder} className="font-mono" autoFocus />
        </Field>
        {error && <div className="mt-3 text-sm text-foreground border border-foreground rounded-md px-3 py-2">{error}</div>}
      </form>
    </Dialog>
  );
}
