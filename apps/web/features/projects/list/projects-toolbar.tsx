...
export function ProjectsToolbar({ query, onQuery, sort, onSort }: { query: string; onQuery: (q: string) => void; sort: SortId; onSort: (s: SortId) => void }) {
  ...
  return (
    <div className="mt-4 mb-4 flex flex-col gap-3 md:mt-7 md:mb-6 md:flex-row md:items-center">
      <label htmlFor={id} className="relative block w-full max-w-[640px]">
        ...
        <input
          ...
          className="h-12 w-full rounded-lg border border-[#2a2a2a] bg-[#101010] pl-10 pr-11 text-sm text-foreground placeholder:text-muted-foreground focus:border-[#525252] focus:outline-none focus:ring-2 focus:ring-white/[0.06] md:h-11 [&::-webkit-search-cancel-button]:hidden"
        />
        {query && <button type="button" aria-label="Clear search" onClick={() => onQuery("")} className="absolute right-1 top-1/2 flex size-10 -translate-y-1/2 items-center justify-center rounded-md text-secondary hover:text-foreground focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-foreground"><X className="size-4" strokeWidth={1.75} /></button>}
      </label>
        ...
        <Menu
          ...
          trigger={({ open, toggle, id: menuId }) => (
            <button
              ...
              onClick={toggle}
              className="flex h-12 w-full items-center gap-2.5 rounded-lg border border-[#2a2a2a] bg-[#101010] px-3.5 text-sm text-foreground hover:border-border-hover focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-white/[0.06] md:h-11 md:w-auto md:min-w-48"
            >
