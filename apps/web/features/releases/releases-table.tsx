    ...
    <>
      <DataTable
        ...
        columns={[
          ...
          { key: "name", label: "Name", width: 12, hide: "lg", render: (r) => <Cell muted title={r.name ?? r.body ?? undefined}>{r.name ?? r.body ?? r.tag}</Cell> },
          { key: "author", label: "Author", width: 17, hide: "lg", render: (r) => <Cell muted title={r.author ?? undefined}>{r.author ?? "—"}</Cell> },
          { key: "sha", label: "Commit", width: 10, hide: "xl", render: (r) => r.sha ? <span className="inline-flex h-6 items-center rounded border border-border bg-background px-1.5 font-mono text-xs">{r.sha.slice(0, 7)}</span> : <Cell muted>—</Cell> },
          { key: "date", label: "Published", width: 13, hide: "md", render: (r) => <Cell muted>{formatDate(r.date)}</Cell> },
