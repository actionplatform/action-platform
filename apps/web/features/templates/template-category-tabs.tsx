...
export function TemplateCategoryTabs({ tabs, active, onChange }: { tabs: { id: string; label: string; count: number }[]; active: string; onChange: (id: string) => void }) {
  return (
    <div role="tablist" aria-label="Template categories" className="mt-3 flex gap-2 overflow-x-auto pb-1 [-ms-overflow-style:none] [scrollbar-width:none] [overscroll-behavior-inline:contain] md:mt-4 [&::-webkit-scrollbar]:hidden">
      {tabs.map((t) => {
        ...
        return (
          <button
            ...
            className={cn(
              "flex h-10 shrink-0 items-center gap-2 whitespace-nowrap rounded-[7px] border px-3.5 text-sm transition-colors md:h-[38px] focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-foreground",
