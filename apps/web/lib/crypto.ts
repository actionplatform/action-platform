...
export function decrypt(sealed: string): string {
  ...
  if (v !== "v1" && v !== "v2") throw new Error("unknown ciphertext version");
  const decipher = createDecipheriv("aes-256-gcm", v === "v1" ? legacyKey() : key(), Buffer.from(iv, "base64url"));
