...
apps/api/app/  (package `app`, depends on the library, never the other way round)
  api/            FastAPI: app (factory, AP_API_TOKEN middleware, Sentry), gate (the /api/v1 ASGI gate: body, scope rewrite, threadpool — the decisions live in `services/access/planner`), dependencies (access, services, lookups, typed: CallerDep, OrgDep, WritesDep… so a route declares what it needs), ratelimit, routers — one package per domain: auth (sessions, organizations, invitations, device, tokens), organizations (identity, members, invitations, settings, template_sources), hosts (connections, oauth_apps, oauth_flow, github_app), projects (projects, apps, organization_import), apps (state, lifecycle, flow, configuration), catalog, jobs
  core/           abc (HostProvider, HostDirectory, ImportSource: one implementation per code host), access (the permission rules per route), auth (AuthService, crypto, jwt, passwords, secrets, cookies), db (models, database, migrations), shared (clock, ids, urls, http, credentials, git_auth)
  repositories/   registry (`registry` table: rows only — cloning is `services/workspace/checkout.Workspaces.adopt`), drafts (`draft` table: pending edits), config_store (`app_config` table: the tables of platform.toml, injected into the services that read them), source (the registry this process opens)
  services/       access (Caller, Authorizer: role ∩ scope ∩ reach, Dispatcher: async and import jobs, Planner: one /api/v1 call decided, request enrichment), projects (what happens to a project's apps: add, init, delete, sync), tokens (TokenMinter), directory (organizations, projects, teams, invitations, hosts_reads / hosts_writes, oauth_apps, template_sources), apps (inventory, scaffolding, remote), workspace (disposable clones, manifest, configuration, commit, flow, state, lifecycle), hosts (one HostProvider per code host, OAuth state, access report, HostConnector), activity (releases and pull requests, one ImportSource per host), organization_import (gateway, preview and the import steps), catalog (sources: which repository and the matrix view; service; published index), plugins (manager, options, catalog), jobs (queue, context, handlers — one callable per job kind), deploy (the environment a deploy job carries), identity (issuer, signer)
  schemas/        the Pydantic models behind the OpenAPI contract, one module per domain (common, auth, organizations, hosts, projects, imports, apps, actions, catalog, plugins, identity); routers import from the owning module
  cli/            `action-platform-api serve | worker | db` — the process entry points, the only place that composes services
  worker.py       `action-platform-api worker`: claims queued jobs and dispatches to `services/jobs/handlers`

```

The library has the same shape: `core` reaches plugins only through `core/extensions.py` (a `Protocol` — hooks after release, deploy and pull request; disabled packages; overlay roots) that `action_platform/__init__.py`, the composition root, wires to the plugin registry. Rule of the layers: `routers → services → repositories/integrations` — enforced by `.importlinter` (`lint-imports` in the code-quality workflow), which also keeps `fastapi` out of services and repositories, the library out of `app`, and `core` off `mcp`/`cli`/`remote`. A service never imports FastAPI: it refuses with a domain error from `core/errors.py` (`Invalid`, `Forbidden`, `NotFound`, `Conflict`, `Gone`, `NeedsInstall`, `Upstream` — every one an `ActionPlatformError` with a `status`), and one exception handler in `api/app.py` (and the gate) turns it into the HTTP answer. The worker runs the same services and sees plain exceptions.
