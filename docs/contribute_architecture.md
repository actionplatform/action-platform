...
action_platform/
  ...
  bootstrap.py    `bootstrap(component)`: the start of every entry point (CLI, MCP server, API serve/worker/db and the uvicorn factory) — `.env`, then Sentry; `project(root)`: the facade for a project on disk
...
apps/api/app/  (package `app`, depends on the library, never the other way round)
  ...
  core/           abc (HostProvider, HostDirectory: one implementation per code host — releases and pull requests come from the library's SourceHost), access (the permission rules per route), auth (crypto, jwt, passwords, secrets, cookies — the service is `services/auth`), db (models — one module per context: auth, organization, projects, configuration, activity, integrations, jobs —, database, migrations), shared (clock, ids, urls, http, credentials)
  repositories/   data access by context, each exposing one composed class on a session: workspace (registry rows, drafts, source), configuration (ConfigStore), organization (OrganizationRepository: organizations, members, teams, invitations), projects (ProjectsRepository: projects, apps), integrations (code hosts, CI hosts, OAuth apps, template sources), releases (ReleaseStore: one row per tag); `base.DirectoryBase` is the session they share
  services/       access (Caller, Authorizer: role ∩ scope ∩ reach, JobDispatcher: async and import jobs, Planner: one /api/v1 call decided, request enrichment, AccessDirectory: the one session composition the gate and the jobs read), projects (service; apps: inventory, scaffolding, remote; organization_import: gateway, preview, the import steps and ImportDirectory — the one place that spans organization, projects and integrations), auth (AuthService: accounts, sessions, first organization, device flow, API tokens), organization (sessions: TokenMinter), workspace (disposable clones, manifest, state, app_context: an app's repository, configuration and tags for the other contexts, git_auth: credentials applied to a config and to git for one call), activity (git-flow on the clone; imports: releases, pull requests and the clone's tags through the library's providers), ci (runner of an app, imported runs), releases, deployments (service, env, identity, records: every delivery of a release to a target), configuration (service, commit), integrations (hosts: one HostProvider per code host, OAuth state, access report, HostConnector, FreshCredentials: the token refresh, IntegrationsDirectory: the repositories plus the refresh on one session; plugins: the bundled ones and their per-organization options), templates (sources: which repository and the matrix view; service; published index), jobs (queue, context, registry — kind → handler, filled by the contexts —, handlers, worker), deploy (the environment a deploy job carries), identity (issuer, signer)
...
Every process is a class in a slot of `core/wiring.py` (`gitflow_rules`, `gitflow`, `releaser`, `deployer`, `readiness`, `installer`, `scaffolder`); callers write `wired.releaser(config, repo)` and a plugin may put a subclass in the slot. `wired` is the process's `Wiring` (`Wiring.default()`), where installed plugins replace slots; `ActionPlatform(config, wiring=...)` takes another, and every process the facade starts resolves through it — a test builds its own `Wiring()` instead of replacing slots in the shared one. Named providers — deploy targets, CI runners, source hosts, release strategies, changelog renderers — come from entry-point groups (`core/module.py`) and platform.toml picks one by name; the core's own (`semver`, `conventional`) live in `core/release/strategies.py`. `plugins/` discovers the `action_platform.plugins` group, keeps the on/off state, registers tools and commands and calls the lifecycle hooks — see [plugins](use_plugins.md) and [writing a plugin](contribute_plugins.md).
...
flowchart LR
    ...
    PL -->|Prefer: respond-async| D[JobDispatcher → job row]
    D -->|202 job id| C
    W[worker] -->|claim| D
...
The Planner (`services/access/planner.py`) is the one place the gate reads the database: who calls, which app, whether the call becomes a job. The JobDispatcher refuses a second `deploy` to a stage that already has one live (`409`) and records whether the caller manages the organization — the deploy token then carries `org.manage`, which the cloud side may require to register the app.
