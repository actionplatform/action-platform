...
| — | — | A `.env` in the working directory is read on start for every variable the shell did not set (the CLI, the MCP server, the API and the worker alike) — by the entry point, never by importing the library. |
