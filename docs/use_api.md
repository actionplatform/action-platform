...
`action-platform-api serve` — the JSON API the web app drives; a FastAPI process listening on `:7788` with OpenAPI at `/docs`. It is released as its own component (`api/vX.Y.Z`, image `actionplatformio/action-platform-api`) and reports that version in `GET /api/version`.

`action-platform-api serve` — one process that refuses to start without `AP_DATABASE_URL`. The API keeps no state on disk. What the app pages read — detail, git-flow health, commits, branches, tags, releases, manifest — comes from `app_snapshot`, a row taken from the clone after every change the platform makes (sync, release, push, branch, checkout, commit, configuration) and on first sight; `GET` routes never touch git. Requests that change something clone fresh (a disposable clone under `AP_WORKSPACES`, brought level with the remote at most every `AP_WORKSPACE_TTL` seconds), apply the app's pending edits from the database and work there. Accounts, sessions, organizations, apps, pending edits and tokens live in the [database](concept_database.md) the API owns. Which organization, project or app a caller may touch is decided here too: `/api/v1/*` is the same API behind a gate that knows who is calling.
...
| `POST /api/apps/{id}/release {level, branch, dry_run, name, notes, latest}`, `/deploy {scope, version, dry_run, force}` (`stage` is accepted as the scope's name; the scope has to exist), `GET /diagnose` | release (fast-forwarded first; a refused push undoes commit and tag), deploy, diagnose; `dry_run` defaults to true; a deploy of a release whose [readiness](#readiness) for the stage is blocked answers `409` unless `force` is true |
...
| `GET projects/{id}/apps/{id}/scopes` (the app's scopes with the vocabulary of kinds, criticalities and executors), `POST … {name, kind, criticality, target, options, run_by, url}`, `PUT/DELETE …/scopes/{name}` (delete refused while a deploy to it runs) | read: reach; writes: `app.configure` |
| `GET projects/{id}/apps/{id}/releases/{tag}/readiness` (the checks per scope), `POST … {stage?}` (queue a check for one stage or for `dev` and `prod`; answers the jobs and the rows, now *pending*) | read: reach; `POST`: `app.release` |
